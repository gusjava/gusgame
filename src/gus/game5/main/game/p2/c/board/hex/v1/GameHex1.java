...
import static gus.game5.main.game.p2.c.board.hex.v1.UtilHex.EMPTY;
import static gus.game5.main.game.p2.c.board.hex.v1.UtilHex.RED;
import static gus.game5.main.game.p2.c.board.hex.v1.UtilHex.searchWinner;
...
//				radioMenuItem("Computer (Min-Max)", ()->changeMode2(Mode.MINMAX))
	...
	public class Cell extends ShapeCell {
		...
		protected void drawShape() {
			...
			double r = getWidth()*0.5;
			double c = getWidth()/Math.sqrt(3);
		...
		protected void initAnchor() {
			int k = i+j;
			double px = gameWidth()*0.5 + getWidth()*Math.sqrt(3)*(j - 0.5*k);
			double py = gameHeight()*0.5 + getWidth()*0.5*(k - x);
